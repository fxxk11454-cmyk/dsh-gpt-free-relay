...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      title: '机场中转',
      desc: '解析订阅，在本机起代理，让 DSH 的模型请求走你的线路。并发固定为 1，不传工具调用。',
      mode: '模式',
      modeNormal: '普通端',
      modeVnc: '浏览器作答',
      webOpen: '在新标签打开',
      ...
      vncReconnect: '重新连接',
      winView: 'Windows 用 CDP 截图当远程画面（没有 Xvfb/x11vnc）',
      winTap: '点画面 = 在页面上点一下',
      winKeys: '键盘',
      winSend: '发送',
      winEnter: '回车',
    ...
    const en = {
      title: 'Relay',
      desc: 'Parse a subscription, run a local proxy, and route DSH model requests through your line. Concurrency fixed at 1, tool calls stripped.',
      mode: 'Mode',
      modeNormal: 'Direct',
      modeVnc: 'Browser',
      ...
      winView: 'On Windows the remote view uses CDP screenshots (no Xvfb/x11vnc)',
      winTap: 'Tap the frame to click in the page',
      winKeys: 'Keyboard',
      winSend: 'Send',
      winEnter: 'Enter',
    ...
    function Card(props) {
      ...
      const [frame, setFrame] = react.useState(0)      // 帧计数器，刷新用
      const [keys, setKeys] = react.useState('')       // Windows 下要敲的字
       ...
       * Windows 专用的远程画面。
       *
       * Windows 没有 Xvfb / x11vnc，VNC 那套起不来，所以改用 Playwright
       * 原生的截图 + 输入注入：画面是一张 <img> 定时刷新，点它就换算成
       * 视口坐标发回去点击。Linux 不走这条分支，仍用 noVNC。
       */
      const isWin = (status && status.platform) === 'win32'
      const winPanel = !isWin
        ? null
        : [
            h(
              'div',
              { style: S.row, key: 'winbar' },
              h('span', { style: S.hint }, t('winView')),
              h(
                'button',
                {
                  style: S.btn,
            ...
            br.running
              ? (() => {
                  ...
                  const onClick = (e) => {
                    ...
                    run(() => call('/browser/tap', { x, y }))
                  }
                  return h('img', {
                    key: 'winframe',
                    alt: 'frame',
                    src: `${API}/browser/frame?q=55&n=${frame}`,
                    onClick,
                    style: { width: '100%', borderRadius: '12px', border: BORDER, background: '#fff', cursor: 'pointer' },
                  })
                })()
              : h(
                  'div',
                  {
                    key: 'winempty',
                    style: {
                      height: '26vh', display: 'flex', alignItems: 'center', justifyContent: 'center',
                      border: BORDER, borderRadius: '12px', fontSize: '13px', opacity: 0.7, textAlign: 'center', padding: '0 16px',
                    },
                  },
                  t('vncIdle'),
                ),
            h(
              'div',
              { style: S.row, key: 'winkeys' },
              h('span', { style: S.label }, t('winKeys')),
              h('input', {
                style: S.input,
                value: keys,
                placeholder: '在这里打字，然后点「发送」',
              ...
              h(
                'button',
                {
                  ...
                  onClick: () =>
                    run(async () => {
                      const r = await call('/browser/keys', { text: keys })
                      if (!r || r.error || r.__error) return r
                      setKeys('')
                      setFrame((n) => n + 1)
                      return r
                    }),
              ...
              h(
                'button',
                {
                  ...
                  onClick: () =>
                    run(async () => {
                      const r = await call('/browser/keys', { key: 'Enter' })
                      setFrame((n) => n + 1)
                      return r
                    }),
                },
            ...
            h('div', { style: S.hint, key: 'winnote' }, t('winTap')),
          ]
      ...
      const body = [switcher, h('div', { style: S.sep, key: 'sep0' })]
        ...
        .concat(mode === 'normal' ? normal : isWin && winPanel ? winPanel : vnc)
