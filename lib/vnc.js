 ...
 * noVNC 与 ws 从哪加载。
 *
 * 不能写死 /root —— Windows 上没有这个路径。这个模块本身是 Linux 专用的
 * （靠 x11vnc，Windows 没有对应物），但路径推导仍该按平台来，免得在
 * 非 Linux 上报一个看不懂的错。
 */
const RUNTIME_DIR = process.env.DSH_BROWSER_HOME || join(homedir(), '.dsh-browser')

/** 这套远程画面依赖 x11vnc + Xvfb，只有 Linux 有。 */
export const VNC_SUPPORTED = process.platform !== 'win32'
...
export class VncBridge {
  ...
  start() {
    ...
    /**
     * Windows 上没有 x11vnc，spawn 只会失败。这里直接说清楚而不是让它
     * 打一行看不懂的错 —— Windows 的远程画面走 CDP 截图（/browser/frame），
     * 根本不经过这个模块。
     */
    if (!VNC_SUPPORTED) {
      this.lastError = 'Windows 没有 x11vnc，远程画面请用 CDP 截图（/browser/frame）'
      return false
    }
    try {
      this.proc = spawn(
