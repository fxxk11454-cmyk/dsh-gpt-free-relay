...
import { buildTurnText } from './toolbridge.js'
 ...
 * 改这里就能换说法，不用动别处。
 */
export const THINKING_PREFIX = '请你使用thinking模式'
 ...
 *
 * 规则（组装逻辑在 toolbridge.js 的 buildTurnText，这里只是包一层默认值）：
 *   - 只发**尾巴**：最后一条 assistant 之后的内容。普通一轮就是那条 user 消息；
 *     工具那一轮是 DSH 带回来的 role:"tool" 结果。历史不重灌 ——
 *     浏览器里是同一个真实会话，ChatGPT 自己记着。
 *   - system / developer 提示没有对应概念，拼在最前面。
 *   - 开了思考再在最前面加一句前缀（见 THINKING_PREFIX）。
 *   - 带 tools 时最前面插一段工具调用协议（见 toolbridge.js）。
 *
...
export function buildPromptText(messages, opts = {}) {
  return buildTurnText(messages, {
    thinking: Boolean(opts.thinking),
    prefix: opts.prefix === undefined ? THINKING_PREFIX : opts.prefix,
    tools: opts.tools,
  })
}
