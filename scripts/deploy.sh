...
#
# 这个文件只是**外壳**：检测 node 是否可用，然后把参数原样转给共用的
# 跨平台核心 scripts/deploy.mjs。真正的逻辑在那边，Windows 用的是同一个。
#
# 用法：
#   bash scripts/deploy.sh                    # 完整部署
#   bash scripts/deploy.sh --no-browser       # 不装浏览器环境
#   bash scripts/deploy.sh --profile web      # 指定 profile
#   bash scripts/deploy.sh --dry-run          # 只看会做什么，不改动
...
case "$(uname -s)" in
  ...
  MINGW*|MSYS*|CYGWIN*)
    ...
    echo "  Windows 请用 scripts\\deploy.bat" >&2
    exit 1
    ;;
