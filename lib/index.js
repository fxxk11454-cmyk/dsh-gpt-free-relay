...
export function apply(ctx, config = {}) {
   ...
   * 不强制绑定机场。
   *
   * 早先这里是「有保存的节点就自动连上」，理由是直连 chatgpt.com 会被
   * Cloudflare 回 403。但这等于替用户做主了 —— 用户可能想先不连、
   * 先用浏览器作答（它不经过机场），或先干别的。改成只在显式
   * `autoConnect: true` 时才自动连。要连自己点「连接」。
   */
  if (config.autoConnect === true && state.nodes.length) {
    setTimeout(() => {
...
export function createRelayState(options = {}) {
  ...
  /**
   * 文本工具协议：把 DSH 发来的 tools 写成一段说明交给网页版，再把模型吐的
   * JSON 翻译回标准 tool_calls（见 chatgpt/toolbridge.js）。
   *
   * **默认关**。「不透传工具调用」是这个插件原本的硬约束，要开得显式开：
   *   curl -X POST 127.0.0.1:2083/tools -d '{"toolsMode":true}'
   *
   * 开了之后约束本身也没破：上游（chatgpt.com）依旧收不到 tools 字段，
   * tool_calls 只在本地合成。
   */
  let toolsMode = options.toolsMode === true
  ...
  function applyLocalHandler() {
    ...
    relay.setLocalHandler({
      match: isChatPath,
      // 开关是活的 —— 运行中改 toolsMode 立刻生效，不用重建 handler
      keepTools: () => toolsMode,
      handle: createLocalChatHandler({ webchat, driver, log, allowTools: () => toolsMode }),
    })
  }
  applyLocalHandler()
  ...
  async function connect(indexOrName) {
     ...
     * 连上机场后，让浏览器改走机场出口。
     *
     * 容器本机直连出去的 IP 是机房 IP，ChatGPT 一律判定为「VPN」并拦。
     * 机场节点是干净的住宅 IP，走它 ChatGPT 才放行。所以「连机场」对
     * 浏览器链路不是可选的优化，是让它能用的前提。
     *
     * 用户没显式配 browserProxy 时才接管（配了就尊重用户的配置）。
     */
    if (options.browserProxy === undefined) {
      browser.setProxy(`http://127.0.0.1:${HTTP_INBOUND_PORT}`).catch((e) =>
        log(`浏览器切到机场出口失败：${String(e?.message || e)}`),
      )
    }
   ...
   * 断开机场连接。
   *
   * **只停 Xray，不关中继。** 中继（2082）是 DSH 的 provider 端点，
   * 关了它整个模型链路就断了 —— 断开只是「不再走机场」，不是「整个下线」。
   * 断开之后：
   *   - 浏览器作答（useWebSession）照常可用，因为它根本不经过机场；
   *   - 机场直连模式会拿到 502（上游没了），这是预期行为。
   *
   * 顺手刷新一下浏览器页面：页面可能是在经机场加载的，断开后那个连接
   * 上下文已经失效，不刷新会停在旧内容上，看着像「断开没作用」。
   *
   * 不在这里调 relay.close()：那会把 `closed` 置真，之后的 connect()
   * 里 relay.listen() 直接拒绝，等于再也连不上了。
   */
  async function disconnect() {
     ...
     * 断开后让浏览器回直连。
     *
     * 连的时候把浏览器切到了机场出口，现在机场没了，得切回去，否则
     * 浏览器还往一个已经死掉的 2081 发请求，整页都打不开。
     * setProxy('') 会重启浏览器并以直连重新加载 —— 这也正好是
     * 用户要的「断开后自动刷新页面」。
     * 用户自己配了 browserProxy 就不动（尊重配置）。
     */
    if (options.browserProxy === undefined) {
      try { await browser.setProxy('') } catch (e) {
        log(`浏览器切回直连失败：${String(e?.message || e)}`)
        try { await browser.reload() } catch { /* 刷新失败不影响断开 */ }
      }
    } else if (browser.running) {
      try { await browser.reload() } catch { /* 刷新失败不影响断开 */ }
    }
    log('已断开机场连接（Xray 已停，中继保留）')
    return { ok: true, coreRunning: false }
  }
  ...
  function startAdmin() {
    ...
    admin = http.createServer((req, res) => {
      ...
      try {
         ...
         * 断开：GET 和 POST 都收。
         *
         * 早先只认 POST，用 curl 或浏览器地址栏试 GET 会拿到 404「未知路由」，
         * 看着像「报错」。断开是幂等的、不带请求体，收 GET 没有副作用。
         *
         * disconnect 是 async（要刷新浏览器页面），必须 await —— 否则 send
         * 拿到的是 Promise，JSON.stringify 成 "{}"，真正的断开动作没人等。
         */
        if (route === 'POST /disconnect' || route === 'GET /disconnect') {
          return (async () => {
            try {
              return send(200, await disconnect())
            } catch (e) {
              return send(500, { error: String(e?.message || e) })
            }
          })()
        ...
        /**
         * 助手消息的真实 HTML —— 调选择器用的只读探针。
         * thinking 块、回答块的结构一改版就变，先看真的再写选择器，
         * 比猜快得多：curl -s '127.0.0.1:2083/browser/turn?max=4000'
         */
        if (route === 'GET /browser/turn') {
          return collect(async () => {
            try {
              return send(200, await browser.dumpTurn(Number(url.searchParams.get('max')) || 20000))
            } catch (e) {
              return send(500, { error: String(e?.message || e) })
            }
          })
        ...
        if (route === 'POST /chatgpt/toggle') {
          return collect((body) => {
            try {
              ...
              return send(200, { ok: true, useWebSession, localChat: Boolean(useWebSession), toolsMode })
            } catch (e) {
              return send(500, { error: String(e?.message || e) })
            }
          })
         ...
         * 文本工具协议开关。
         *
         * 开了之后，带 tools 的请求会走 toolbridge.js 那条路：工具清单被写成
         * 一段说明塞进提问，模型吐 JSON，我们再翻译回标准 tool_calls。
         * 上游依旧看不到 tools 字段，「不透传工具调用」的约束没破，只是本地
         * 多了一层翻译。
         */
        if (route === 'GET /tools') return send(200, { ok: true, toolsMode })
        if (route === 'POST /tools') {
          return collect((body) => {
            try {
              const p = body ? JSON.parse(body) : {}
              if (typeof p.toolsMode === 'boolean') toolsMode = p.toolsMode
              else toolsMode = !toolsMode
              applyLocalHandler()
              log(`文本工具协议：${toolsMode ? '开' : '关'}`)
              return send(200, { ok: true, toolsMode })
            } catch (e) {
              return send(500, { error: String(e?.message || e) })
            }
          })
