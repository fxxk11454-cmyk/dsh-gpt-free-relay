...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 网页端卡片里 iframe 指向的地址（host 端的网站穿透路由） */
    ...
    const zh = {
      ...
      working: '处理中…',
      webTitle: '机场中转 · 网页端',
      webDesc: '直接在卡片里渲染官网，流量经你的机场线路；用于调试与直接使用。',
      webOpen: '在新标签打开',
      webReload: '重新加载',
    ...
    const en = {
      ...
      working: 'Working…',
      webTitle: 'Relay · Web',
      webDesc: 'Renders the official site inside this card, through your line.',
    ...
    function WebCard(props) {
      ...
      const header = h(
        ...
        h(
          'span',
          { style: { flex: '1 1 0%', minWidth: 0 } },
          h('div', { style: { fontSize: '14px', fontWeight: 500 } }, t('webTitle')),
          h('div', { style: { marginTop: '3px', fontSize: '12px', opacity: 0.7, lineHeight: 1.5 } }, t('webDesc')),
        ),
        h('span', { style: { flex: 'none', opacity: 0.5, fontSize: '12px' } }, open ? '▾' : '▸'),
      )
      ...
      const src = WEB_URL + (nonce ? '?n=' + nonce : '')
      return h(
        'div',
        { style: S.card },
        header,
     ...
     */
    function apply(ctx) {
      // 心跳：让 host 端知道浏览器端确实执行到这里（排查界面不出现用）
      const beacon = (note) => {
        try {
      ...
      ctx.slots.inject('settings.plugin.item', function* () {
        ...
        // 网页端：卡片内直接渲染官网（走同一套代理）
        yield ctx.slots.register(
          { name: 'settings.plugin.item', key: SETTINGS_NAMESPACE + '-web', locale: NS, inject: injected },
          WebCard,
        )
