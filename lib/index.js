...
export function apply(ctx, config = {}) {
  ...
  log(`已就绪；管理接口 http://127.0.0.1:${options.adminPort || ADMIN_PORT}/status`)
  return state
}
...
export function createRelayState(options = {}) {
   ...
   */
  const session = new WebSession({ log })
  ...
  async function proxySite(req, res, targetUrl) {
    ...
    for (const k of ['cookie', 'accept-language', 'cache-control', 'pragma', 'content-type']) {
    ...
    admin = http.createServer((req, res) => {
      ...
      try {
        ...
        if (url.pathname === '/web' || url.pathname.startsWith('/web/')) {
          const web = new URL(options.webTarget || DEFAULT_WEB_TARGET)
