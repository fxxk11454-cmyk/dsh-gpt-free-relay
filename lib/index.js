...
/** 注册到「模型」区的 provider id 与默认模型 */
const PROVIDER_ID = 'gpt-free-relay'
const DEFAULT_MODELS = ['gpt-4o-mini', 'gpt-4o', 'gpt-4.1-mini', 'deepseek-chat']
      ...
      await ctx.settings.mutate('llm-pi-ai', [
        {
          ...
          value: {
            ...
            apiKey: 'relay-managed',
            models: (models && models.length ? models : DEFAULT_MODELS).map((id) => ({ id })),
          },
        },
      ])
      log(`已把 provider 注册进 llm-pi-ai（baseURL=http://127.0.0.1:${relayPort}/v1）`)
      return null
    } catch (e) {
