...
export class XrayCore {
  ...
  async start() {
    ...
    if (!this.corePath) {
      return '未找到 Xray 核心。Linux 跑 bash setup.sh（或 bash scripts/linux/fetch-core.sh），Windows 跑 setup.bat'
    }
  ...
  stop() {
    ...
    if (!p) return
    /**
     * 先 SIGTERM，500ms 没退就 SIGKILL。
     *
     * 早先后备是 1500ms，太慢：用户点「断开」要等一秒多才看到 Xray
     * 真正消失，期间界面像是「没作用」。实测 Xray 对 SIGTERM 响应很快
     * （几百毫秒内退），500ms 兜底足够。
     *
     * p 可能已经自己退了（exitCode !== null），再 kill 只是无操作，
     * 包在 try 里就行。
     */
    try {
