...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    const DEFAULT_WEB_URL = 'https://chatgpt.com'
    ...
    const zh = {
      ...
      vncNote: '这是容器里真实运行的 Chromium（有头模式），Cloudflare 认它是真浏览器，不会发验证。直接在这里操作登录即可 —— 登录后中转就能用它作答。',
      sub: '订阅地址',
      fetch: '拉取并解析',
      node: '节点',
      model: '模型',
      ...
      saved: '已保留数据',
      cgOk: '网页端已登录',
      cgNo: '网页端未登录 —— 请切到「网页端」标签，在那个页面里登录 ChatGPT',
      cgRefresh: '刷新登录态',
      cgClear: '清空会话',
      ...
      webReload: '重新加载',
      webOpen: '在新标签打开',
      webNote: '这里直接加载真实站点，不经过本地中转。若显示空白，是该站点禁止被嵌入 —— 点「在新标签打开」即可，登录与验证都在那边完成。',
    }
    const en = {
      title: 'Relay',
      ...
      vncNote: 'A real headed Chromium running in the container. Cloudflare treats it as a real browser, so no challenge. Sign in right here — the relay then uses it.',
      ...
      working: 'Working…',
      saved: 'Kept data',
      cgOk: 'Web side signed in',
      cgNo: 'Web side not signed in — switch to the Web tab and sign in to ChatGPT there',
      ...
      webNote: 'Loads the real site directly, no local relay. If blank, the site blocks embedding — use “Open in new tab”; sign-in and checks happen there.',
    }
    ...
    function Card(props) {
      ...
      const [webUrl, setWebUrl] = react.useState(DEFAULT_WEB_URL)
      ...
      normal.push(
        h(
          ...
          h(
            'span',
            { style: cg && cg.loggedIn ? S.status : { ...S.status, color: 'var(--dsw-alias-state-warning-primary, #d08700)' } },
            cg
              ? cg.loggedIn
                ? `${t('cgOk')}${cg.email ? `（${cg.email}）` : ''} · ${t('cgModel')} ${cg.webModel}`
                : t('cgNo')
              : t('loading'),
          ),
          h(
            'button',
            { style: S.btn, disabled: busy, onClick: () => run(() => call('/chatgpt/refresh', { force: true })) },
            t('cgRefresh'),
          ),
          h(
            'button',
      ...
      const sites = [
        ...
        ['login', t('wtLogin')],
        ['openai', t('wtOpenai')],
        ['platform', t('wtPlatform')],
      ]
      const siteRow = h(
        'div',
        { style: S.row, key: 'site' },
        h('span', { style: S.label }, t('wtSite')),
        ...sites.map(([key, label]) =>
          h(
            'button',
            {
              key,
              style: webUrl === PRESET_URL[key] ? S.btnMain : S.btn,
              onClick: () => {
                setWebUrl(PRESET_URL[key])
                setNonce(Date.now())
              },
            },
            label,
      ...
      const web = [
        ...
        h(
          ...
          h(
            'a',
            { style: { ...S.btn, textDecoration: 'none', display: 'inline-block' }, href: webUrl, target: '_blank', rel: 'noreferrer' },
            t('webOpen'),
          ),
          h('span', { style: S.hint }, webUrl),
        ),
        h('iframe', {
          key: 'frame',
          src: webUrl,
       ...
       * 这里只负责把话说清楚，并且一键把你送到能过验证的地方（网页端）。
       */
      ...
      const banner = !needClearance
        ...
        : h(
            ...
            mode === 'normal'
              ? h('button', { style: S.btnMain, onClick: () => setMode('web') }, t('cgGoWeb'))
              : h(
                  'button',
      ...
      const body = [switcher, h('div', { style: S.sep, key: 'sep0' })]
        ...
        .concat(mode === 'normal' ? normal : mode === 'vnc' ? vnc : web)
