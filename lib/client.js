...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 站点预设 → 域名片段，用来把当前站点对应到某个预设按钮上 */
    const PRESET_HOST = {
      chatgpt: 'chatgpt.com',
      login: 'auth.openai.com',
      openai: 'openai.com',
    ...
    const zh = {
      ...
      cgNoClearance: '尚未取得免验证凭证（可能需要先去「网页端」过一次验证）',
      cgModel: '网页模型',
      cgLocal: '请求本地处理',
    ...
    const en = {
      ...
      cgNoClearance: 'No clearance cookie yet (you may need to pass a check in the Web tab first)',
      cgModel: 'Web model',
    ...
    function Card(props) {
      ...
      // 被 Cloudflare 挑战：只提示"去哪儿过"，不假装能自动过
      normal.push(
        h(
          'div',
          { style: S.hint, key: 'cgstate' },
          cg && cg.challengedAt
            ? `⚠️ ${t('cgChallenge')}`
            : cg && !cg.hasClearance
              ? t('cgNoClearance')
      ...
      const wt = (status && status.webTarget) || ''
      const sites = [
        ['chatgpt', t('wtChatgpt')],
        ['login', t('wtLogin')],
        ['openai', t('wtOpenai')],
      ...
      const siteRow = h(
        ...
        ...sites.map(([key, label]) =>
          h(
            ...
            {
              key,
              style: wt && wt.includes(PRESET_HOST[key]) ? S.btnMain : S.btn,
              disabled: busy,
              onClick: () => run(async () => {
                const r = await call('/web-target', { preset: key })
                if (!r || r.__error || r.error) return r
                setNonce(Date.now())
                return r
              }),
            },
        ...
        h(
          'button',
          {
            ...
            onClick: () => run(async () => {
              const r = await call('/web-target', { url: siteInput.trim() })
              if (!r || r.__error || r.error) return r
              setSiteInput('')
              setNonce(Date.now())
              return r
      ...
      const web = [
        ...
        h(
          ...
          h('span', { style: S.hint }, `${WEB_URL} → ${wt || '…'}`),
        ),
        h('iframe', {
          key: 'frame',
