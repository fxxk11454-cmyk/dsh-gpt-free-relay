...
export function createRelayState(options = {}) {
  ...
  /**
   * 「有头浏览器」：真 Chromium 跑在 Xvfb 上，经机场出网。
   *
   ...
   */
  const browser = new HeadedBrowser({ log, target: () => webTarget })
  ...
  function applyLocalHandler() {
    ...
    relay.setLocalHandler({ match: isChatPath, handle: createLocalChatHandler({ webchat, log }) })
  }
  applyLocalHandler()
   ...
   * 卡片「网页端」加载哪个站点。
   *
   * 做成可切换的：不同人对"官方站"指的不是同一个地方 ——
   * chatgpt.com 是聊天站本体（默认），auth.openai.com 是登录页，
   * platform.openai.com 是拿 API Key 的地方。
   */
      ...
      try {
        ...
        if (route === 'POST /browser/click') {
          return collect(async (body) => {
            try {
              ...
              try {
                sess = await browser.probeSession()
              } catch {}
        ...
        if (route === 'POST /browser/start') {
          return collect(async () => {
            ...
            return send(r.ok ? 200 : 500, r.ok ? { ...r, ...browser.status, vnc: vnc.running } : r)
          })
        }
        ...
        if (route === 'POST /chatgpt/refresh') {
          return collect(async (body) => {
            try {
              ...
              return send(200, { ...st, useWebSession, webModel: webchat.webModel })
            } catch (e) {
              return send(500, { error: String(e?.message || e) })
            }
          })
